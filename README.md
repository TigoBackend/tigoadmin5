...
```
2018-8-24 移除api应用，app应用新增通用接口模块entry（适用于普通非耗时接口），将api应用中的示例代码移到app应用的entry接口示例模块中。集成友盟、环信service，JWT加密方式的token验证。新增微信模板消息批量推送功能。修复后台微信菜单功能。新增耗时任务应用task，使用方法类似原来的api模块（适用于服务器内部调用耗时业务功能，不对外公开这类接口地址）
